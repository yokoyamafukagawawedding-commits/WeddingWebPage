...
3. 作成済みのGoogleフォームのIDを`SETUP.formId`に設定します。IDはフォームURLの`/d/`と`/edit`の間にある文字列です。
4. `SETUP`の設問名・新郎側/新婦側の選択肢・`colors`を、フォームの内容と完全に同じ表記で設定します。
5. `SETUP.adminKey`を12文字以上の推測されにくい文字列に変更します。
6. Apps Scriptで初回実行時に権限を許可します（WebアプリのURLへアクセスしたときにも許可できます）。

フォームには、以下の回答が含まれるようにしてください。
